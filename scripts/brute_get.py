...
parser.add_argument("--get", help="the name of the get parameter sent in the request", type=str)
...
if args.get:
    ...
    print("Use --get to set the parameter name you want to post (e.g. username)")
    get_param = input()
if args.output:
    output = args.output
else:
...
for word in mylist:
    values = "&" + get_param + "=" + word
    url_req = url + values
    print("############### Trying: " + url + " ###############")
    data = values.encode("utf-8")
    req = urllib.request.Request(url_req, data)
